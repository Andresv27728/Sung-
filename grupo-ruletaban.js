...
let handler = async (m, { conn, participants, isAdmin, isOwner, isROwner }) => {
  ...
  if (!bot?.admin) {
    return conn.reply(m.chat, `${emoji2} Necesito ser administrador para eliminar miembros.`, m)
  }
  ...
  if (membersToRemove.length === 0) {
    return conn.reply(m.chat, `${emoji2} No hay miembros disponibles para eliminar (solo hay admins y el bot).`, m)
  }
...
🎭 Los administradores están a salvo... por ahora
    ...
    await conn.reply(m.chat, `${emoji2} Error al eliminar al miembro. Verifica que tenga los permisos necesarios.`, m)
  }
}
